...
def addNewScenario():
    ...
    cursor = conn.cursor()
    print("I am HEREEEEE " + db_path)
    cursor.execute("insert into Scenarios (UserID, Name, Value, ValueType, Duration, DurationType, Description) " +
                                 " values (?,?,?,?,?,?,?)",
                                 [
                                    current_user.id,
                                    content['name'],
...
def removeScenario():
    ...
    cursor.execute("delete from BasicOutput where ScenarioID = " + str(content["id"]))
    conn.commit()
    conn.close()
